...
/**
 * Merges fork-specific customizations into the upstream defaults WITHOUT
 * modifying the tracked file:
 *
 *   customizable_default.ts    (READ-ONLY static upstream defaults — never written)
 * + customizable_overwrites.ts (READ custom deltas)
 * = customizable_merged.ts     (WRITTEN generated output, git-ignored)
 *
 * All runtime code must import from `customizable_merged`, never from
 * `customizable_default` directly. Regenerate with:
 *   npm run apply-overwrites      (also runs on `prestart` and before `build:prod`)
 */
...
const MERGED_PATH     = path.join(CONFIG_DIR, 'customizable_merged.ts');

const BANNER = `/**
 * AUTO-GENERATED FILE — DO NOT EDIT BY HAND.
 * Generated by eda/eda_app/scripts/apply-overwrites.js from:
 *   - customizable_default.ts    (static upstream defaults, never modified)
 *   - customizable_overwrites.ts (custom fork deltas)
 * Regenerate with: npm run apply-overwrites
 * (also runs automatically on \`prestart\` and before \`build:prod\`).
 */
 ...
 * or up to the end of line for scalar declarations without trailing `;`
 * (e.g. `export const X = '#000000'`), so following declarations are preserved.
 */
function extractDeclaration(src, pos) {
    ...
    while (i < len) {
        ...
        if (depth === 0 && valueStarted && (ch === '\n' || ch === '\r')) {
            // Scalar declaration without trailing `;`:
...
fs.writeFileSync(MERGED_PATH, BANNER + result, 'utf8');
console.log(`\n\x1b[34m=====\x1b[0m Applied \x1b[32m[${count}] override(s)\x1b[0m to customizable_merged.ts \x1b[34m=====\x1b[0m`);
console.log(`\x1b[34m=====\x1b[0m customizable_default.ts left untouched \x1b[34m=====\x1b[0m`);
